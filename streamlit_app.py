...
US_BDAY         = CustomBusinessDay(calendar=USFederalHolidayCalendar())  # weekends + US hols
...
raw_tickers = st.sidebar.text_input(
    "Tickers (allowed: SPY, AGG, VNQ, DBC)", value="SPY,AGG,VNQ,DBC"
).upper()
...
# -- Date range (capped at TODAY)
c1, c2 = st.sidebar.columns(2)
start = c1.date_input("Start date", pd.to_datetime("2018-01-01"), max_value=TODAY)
end_default = min(pd.to_datetime("2024-12-31"), TODAY)  # ensure default ≤ today
end = c2.date_input("End date", end_default, min_value=start, max_value=TODAY)
if start >= end:
...
st.dataframe(
    pd.DataFrame({"Mean Ann. Return": mu_ann, "Volatility (Ann.)": ann_vol}).style.format("{:.2%}")
)
...
if results.size == 0:
    st.error("Simulation failed (all zero‑vol portfolios).")
    st.stop()
...
if run_opt:
    ...
    def neg_sharpe(w):
        return -((w @ mu_ann - rf) / np.sqrt(w @ cov_ann @ w))
...
"""
    ...
    # — REQUIRED REWRITE —
    st.markdown("#### Possible implementation")
    st.markdown("1. **Estimate cost per trade** as a % of notional or a fixed ¢ per share.")
    st.markdown("2. **Adjust expected returns**")
    st.latex(r"\mu_i = \mu_i - c_i")
    st.markdown("before optimization.")
    st.markdown("3. **Display cost‑adjusted frontier** versus the frictionless one.")
